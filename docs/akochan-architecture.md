...
「シャンテン数」は和了までに必要な改善段階です。`-1` が和了、`0` が聴牌、`1` が1シャンテンです。「受け入れ」は、引いたときにシャンテン数を下げる牌種と、その残り枚数を指します。
...
これは理想的な条件付き確率として再正規化した値ではなく、コード上の分布を単純集計した推定量です。副露手牌の列挙側で聴牌率を重みに含めた後、総量の計算でも聴牌率を掛ける経路があります。複数相手に対する総放銃確率は相手間の独立性補正をせず単純加算し、`1.0` で上限処理します。
...
ここで `houjuu_prob` は牌ごとの危険度モデルではありません。牌別危険度は待ち・翻・符の推定分布から作ります。`cal_target_prob()` は、ある他家が和了したときのロン対象を分配します。河4枚以下では自分への確率を `2/11` に固定し、それ以降は `[1, risk_array, 自分の副露数]` を用意しますが、現行呼び出しは特徴数2なので副露数を実際には使いません。残余を和了者自身 `5/9`、それ以外 `2/9` へ配ります。
...
パラメータはプレーンテキストの浮動小数点列です。推論関数が相対パスから必要個数を読み、個数不足やファイル不在では `assert` で停止します。そのため通常はリポジトリルートから実行する必要があります。
...
- MJAIアクションの一部の `is_valid_*` は主に `type` だけを確認し、完全な JSON Schema 検証ではありません。
